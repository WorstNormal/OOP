...
public abstract class AbstractGraph<V> implements Graph<V> {
    ...
    public void readFromFile(String filePath) throws IOException {
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath))) {
            ...
            if (line == null) throw new IOException("File is empty or invalid: missing vertex count.");
            int numVertices = Integer.parseInt(line.trim());

            for (int i = 0; i < numVertices; i++) {
                line = reader.readLine();
                if (line == null) throw new IOException("Unexpected end of file while reading vertices.");
            ...
            for (int i = 0; i < numEdges; i++) {
                line = reader.readLine();
                if (line == null) throw new IOException("Unexpected end of file while reading edges.");
                if (line.trim().isEmpty()) { i--; continue; }

                String[] edgeParts = line.trim().split("\\s+");
                if (edgeParts.length < 2) throw new IOException("Invalid edge format: " + line);
